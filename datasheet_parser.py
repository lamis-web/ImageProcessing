...
MRN_CTDATE_PATH = 'COVID_CTs_20210513CarissaWalter_0608jc.xlsx'
VIDA_PATH_PREFIX = 'E:\\jchoi4\\ImageData\\VIDA_20210608-13_C19_TK\\'
DCM_PATH_PREFIX = ''
...
vida_raw_data_trimmed = vida_raw_data[vida_raw_data['Case ID'] >= start_vida_case_number]
...
def construct_new_rows(vida_raw_data):
    last_row_raw = vida_raw_data.iloc[-1]
    end_index = last_row_raw['Case ID'] + 1

    rows_to_append = []
    for i in range(start_vida_case_number, end_index):
        row = {}
        row_data_raw = vida_raw_data.loc[vida_raw_data['Case ID'] == i]
        ...
        ctdate = row_data_raw['Acquisition Date'].tolist()[0].strftime('%Y%m%d')
        try:
...
# df = construct_new_rows(vida_raw_data_trimmed).sort_values(by=['Subj', 'IN/EX'])
df = construct_new_rows(vida_raw_data_trimmed)
